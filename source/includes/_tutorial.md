...
In this section we will do some common things that many of our users ask for help on how to accomplish them.  We will mostly use the `sync` call, which is described in detailed at the `Sync` section, but for now you can just consider it just an endpoint, and you will soon figure some of the things it can do, just by having a look at some examples.
...
Let's see first how we can get all projects a user has.
...
- A new `sync_token` which we can use later on to do incremental syncs.
...
After that we can just do a sync by calling `api.sync()`, and we can access the user's project through the `api.state` object, so for projects that is `api.state['projects']`.
...
We use again the `/API/v7/sync` endpoint, and then specify the following arguments:

- The user's API token which is again set to `token=0123456789abcdef0123456789abcdef01234567`.
...
- That we again want to get back only the `projects`, and not any other data, so we set `resource_items='["projects"]'`.
...
- We also need to specify 2 UUIDs, one is the `uuid` that that will uniquely identify our command, and one for the `temp_id` which is a temporary ID we set to our new project, and we can use that later on to identify it to the server without a need to know its real ID that is assigned at the time of creation.

In the results we get back, we notice the following data:

- The new project we added is returned as part of the user's `projects`.
- The `temp_id_mapping` object which tells us that the new object with UUID `24a193a7-46f7-4314-b984-27b707bd2331` has a real ID `176637191`, and we can use both of these IDs to refer to that project, and while the latter should be used whenever possible, the former can be also utilized on a temporary basis.
...
We call the `api.projects.add()` call, supplying the name of the new project, and as a return value we get a new project object.  This object holds this new project, and which we store in the `project1` variable.

This object is only temporary, and only exists on our local state, so in order to ask the server to add it remotely on the system, too, we use the `api.commit()` call, which does all the work of sending a request to add the project, and get back the new project's properties.
...
- For this example lets get back both projects and items changed since last sync, so we set `resource_items='["projects", "items"]'`.
- We send two `item_add` commands that will create a new task each, and we also specify the `project_id` and the `content` of each new task, and for one of the tasks we use the `temp_id` of the previously created project to denote the `project_id`, while for the other task we use the project's real ID, and we do that just to show that it has the same result.
...
- The `temp_id_mapping` object which tells us again that are the real IDs of the new tasks, and what were the `temp_id`s we used.
- The `sync_status` object which tells us again whether each command was successful.
- The special flag `full_sync` which is set to `false` here, and denotes we did an incremental sync.
- A new `sync_token` which we can use later on to do more incremental syncs.

#### When using the python library...

We call the `api.items.add()` call, supplying the content each new task and the project it should be added to, and as a return value we get a new item object, that holds the new task, and which we store to a variable.  Notice that we can get the project ID from the `project1` object we created on the previous step, and specifically from the `project1['id']` value.

Again, in order to actually add the tasks on the server, we do an `api.commit()` call, and so both item objects are populated with the properties these new tasks have.
...
- We send an `item_update` command that will update the task we created earlier, so we specify the `id` of the task, its new `content`, and its new due date with `date_string`.
...
- The `sync_status` object which tells us again whether our command was successful.
- The special flag `full_sync` which is set to `false` here, and denotes we did an incremental sync.
- A new `sync_token` which we can use later on to do more incremental syncs.

#### When using the python library...

We call the `update()` call on the `task1` object that we got in the previous step, so we call `task1.update()`, and we specify the new `content=NewTask1` and `date_string='tomorrow at 10:00` parameters, in order to change the task's properties.

Again, in order to update the task on the server, we do an `api.commit()` call.

Finally we print again the `task1` object, which has its properties updated.
...
- The sync token that we received as reply on our previous request.
- For this example lets get back only items changed since last sync, so we set `resource_items='["items"]'`.
- We send an `item_complete` command that will completed the task, and we specify only the `ids` parameter with the ID of the task, and also we send an `item_delete` command that will delete the other task, and again it expects an `ids` parameter.
- We also need to specify the `uuid` for this command.

In the results we get back, we notice the following:

- The updates to all items since our last sync are returned as part of the user's `items` array, where we can observe that for the task we completed it has a `checked=1` property which denotes it's now completed, while the other task has a `is_deleted=1` property which denotes it's now deleted.
- The `temp_id_mapping` here is empty since no new object was created.
- The `sync_status` object which tells us again whether our command was successful.
- The special flag `full_sync` which is set to `false` here, and denotes we did an incremental sync.
- A new `sync_token` which we can use later on to do more incremental syncs.
...
Again, in order to actually completed and delete the tasks on the server, we do an `api.commit()` call.
...
- The updates to all items, notes and reminders, since our last sync are returned as part of the user's `items`, `notes` and `reminders` arrays, and we can observe the new task, and the note and reminder that it has.
- The `temp_id_mapping` here contains the real IDs of the task, note and reminder.
- The `sync_status` object which tells us again whether our commands were successful.
- The special flag `full_sync` which is set to `false` here, and denotes we did an incremental sync.
- A new `sync_token` which we can use later on to do more incremental syncs.

#### When using the python library...

We call the `api.items.add()` method to create the new task. and we store the object in the `task3` variable, which we use to specify the `task3['id']` argument for the `api.notes.add()` and `api.reminders.add()` methods that create the `comment3` note and `reminder3` reminder respectively.

Again, in order to actually create all the objects on the server, we do an `api.commit()` call.
